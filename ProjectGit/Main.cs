                    ...
                    for (int inBatchIndex = currentIndex; inBatchIndex < currentIndex + config_.BatchSize && inBatchIndex < data.Count; inBatchIndex++)
                    {
                        ...
                        for (int j = 0; j < network.Layers[network.Layers.Length - 1].Neurons.Length; j++)
                        {
                            network.Layers[network.Layers.Length - 1].Neurons[j].dEdz =
                                config_.ErrorFunction.calculatePartialDerivaitveByV2Index(data[inBatchIndex].Output,
                                                                                            realOutput, j) *
                                network.Layers[network.Layers.Length - 1].Neurons[j].ActivationFunction.
                                    computeFirstDerivative(network.Layers[network.Layers.Length - 1].Neurons[j].LastSum);

                            nablaBiases[network.Layers.Length - 1][j] += config_.LearningRate *
                                                                        network.Layers[network.Layers.Length - 1].Neurons[j].dEdz;

                            for (int i = 0; i < network.Layers[network.Layers.Length - 1].Neurons[j].Weights.Length; i++)
                            {
                                nablaWeights[network.Layers.Length - 1][j][i] +=
                                    config_.LearningRate * (network.Layers[network.Layers.Length - 1].Neurons[j].dEdz *
